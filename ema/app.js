...
function loadAndRender() {
  ...
  const data = filter
    ? parsedData.filter(r =>
        ...
        r['Indication'].toLowerCase().includes(filter)
      )
    : parsedData;
  // Render
  container.innerHTML = '';
...
function renderByKategorie(data) {
  ...
  Object.keys(groups).sort().forEach(cat => {
    ...
    d.innerHTML = `<summary>${cat}</summary><table>
      <thead><tr><th>Medikation</th><th>Indication</th></tr></thead>
      <tbody>${groups[cat].map(r=>`<tr><td>${r.Medikation}</td><td>${r['Indication']}</td></tr>`).join('')}</tbody>
    </table>`;
...
function renderByMedikation(data) {
  ...
  table.innerHTML = `<thead><tr><th>Medikation</th><th>Kategorie</th><th>Indication</th></tr></thead>
